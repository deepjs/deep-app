...
define(["require", "deepjs/deep", "deep-routes/lib/route"], function(require, deep){
	...
	var appRouters = {
		...
		get:function(uri, opt){
			...
			{
				var router = this.routers[i], matched;
				if(!router.get) // skip statics for the moment
					continue;

				// router.outputType should match opt.acceptType
				// router.inputType should match opt.contentType

				if(matched = router.route.match(uri))
					return router.get(matched.parts.slice(matched.index), opt);
			}
			return null;
		}
	};
