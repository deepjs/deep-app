...
middleware.csrf = function(request, response, next) {
	...
	if(session && session.user && request.header.uuid !== request.session.uuid)
	{
...
middleware.context = function(app){
	return function(request, response, next){
		...
		.fail(function(error){
			...
			response.writeHead(error.status || 400, {'Content-Type': 'plain/text'});
...
middleware.routers = function(request, response, next) {
	...
	.fail(function (e) {
		console.log("App.routers middleware error : ", e.status, e);
		if(e.status == 4000)
			return next();
